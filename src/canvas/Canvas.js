class Canvas {
	 ...
	 */
	setStage({ parentId, width, height, scale, parentHeight, parentWidth }) {
		this.width = Math.round(width * scale);
		this.height = Math.round(height * scale);
		this.scale = scale;

		const canvas = this.createCanvasElement({ id: `canvas`, zIndex: 10, parentHeight, parentWidth });
		this.canvas = canvas;
		this.ctx = canvas.getContext('2d');

		const canvasDraw = this.createCanvasElement({ id: `canvasDraw`, zIndex: 10, parentHeight, parentWidth });
		this.canvasDraw = canvasDraw;
		this.ctxDraw = canvasDraw.getContext('2d');

		const canvasImage = this.createCanvasElement({ id: `canvasImage`, zIndex: 5, parentHeight, parentWidth });
		this.canvasImage = canvasImage;
		this.ctxImage = canvasImage.getContext('2d');

		const canvasOrigin = this.createCanvasElement({ id: `canvasOrigin`, zIndex: 10, parentHeight, parentWidth });
	 ...
	 */
	resizeCanvasAll(width, height, scale) {
		this.zoom = 1;
		...
		this.relocateCanvasAll({ newWidth, newHeight });
	 ...
	 */
	relocateCanvasAll({ newTop, newLeft }) {
